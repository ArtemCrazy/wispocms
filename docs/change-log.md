...
- Статус: **В работе** — пользователь согласовал внешний сетевой выход для проверки.
- Владелец / задача / ветка: задача Артёма / Codex, `docs/deepseek-text-release`.
- Что изменено и зачем: добавлен отдельный сбор публичной карточки продавца Ozon, первых 10 товаров и до 8 текстовых отзывов на товар по ссылке без доступа к кабинету продавца. Для сетевого выхода используется настраиваемый прокси; ошибки не сохраняются как успешный пустой снимок.
- Файлы / модули: `ozon-source.ts`, маршрутизация сбора и обновления, окно источника, Compose overlay, инструкция `deploy/OZON-COLLECTION.md`, тесты и эта запись журнала.
- БД — схема: нет.
- БД — данные и формат: новые снимки Ozon записываются в существующий `cc_materials.site_pages`; миграций нет и старые записи не меняются.
- Совместимость / пересечения: Wildberries, Яндекс Маркет и другие источники не менялись.
- Проверки и оставшиеся ограничения: 5 точечных тестов, сборка API, ESLint затронутых файлов и TypeScript web прошли. Живые публичные товары и отзывы проверены во встроенном браузере, но HTTP с ПК и VDS возвращает циклические 307, а Chromium API на VDS — страницу Ozon «Похоже, нет соединения». До появления прокси и успешного прогона на VDS нельзя считать end-to-end проверенным.
- Коммит реализации: нет.
- Выкладка: не было.
- Восстановление: прежний код игнорирует новый снимок; схема не меняется, а предыдущий успешный снимок не перезаписывается при ошибке сбора.
