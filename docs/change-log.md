...
- БД — схема: нет. БД — данные и формат: миграция обновляет только нетронутый
  стартовый промт «Анализ компании», переименовывает нетронутый конкурентный
  шаблон в «Отзывы и обратная связь» и добавляет «Общая сводка компании»;
...
- Проверки и оставшиеся ограничения: seed-тест и адресная проверка миграции
  должны быть выполнены; авторизованный Browser smoke зависит от доступной
  сессии.
- Коммит реализации: будет указан после проверки.
- Выкладка: не было.
- Восстановление: миграция необратима по данным общего каталога; перед
  выкладкой нужен резервный каталог и штатный откат версии приложения.
