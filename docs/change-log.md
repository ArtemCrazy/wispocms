...
- Проверки и оставшиеся ограничения: 8/8 точечных тестов обновления источников,
  ESLint изменённого TSX-файла и `git diff --check` пройдены. Визуальная
  проверка в авторизованном окне выполняется после выкладки.
- Коммит реализации: ожидается.
- Выкладка: не выполнялась.
- Восстановление: возврат прежнего класса вернёт кнопку в строку пояснения.
...
- Приложение на общем сервере: `1b089194209d334e816f665c025e205ef0f5a3c0`.
