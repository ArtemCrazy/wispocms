...
- Что изменяется и зачем: принимать публичную ссылку 2ГИС вида
  `/город/search/запрос/firm/числовой-id` и собирать каноническую карточку
  `/город/firm/id`; сейчас такая сохранённая ссылка не проходит проверку API.
...
- Проверки и оставшиеся ограничения: профильные тесты (3), адресный ESLint и
  сборка API прошли. Сохранение ссылки само по себе не запускает сбор; нужен
  явный клик «Обновить сбор».
- Коммит реализации: не создан. Выкладка: не было.
- Восстановление: откат к прежней API-версии без изменения БД.
