...
- Проверки и оставшиеся ограничения: unit-тесты AI/DeepSeek, адресные
  интеграционные тесты на отдельной PostgreSQL-базе, API build, web TypeScript,
  адресный ESLint и web-тест прошли. Авторизованный Browser smoke ещё нужен.
- Коммит реализации: ожидается.
- Выкладка: не было.
- Восстановление: перед миграцией нужна резервная копия БД и исходного кода;
  откат API требует удаления новой таблицы/колонки или совместимого старого кода.
