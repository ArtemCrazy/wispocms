...
- Что изменено и зачем: временный сбой соединения, некорректная оболочка ответа
  ...
  запуска увеличен с 15 до 45 минут.
- Файлы / модули: DeepSeek-адаптер и поэтапная AI-подготовка в `apps/api`,
  адресные тесты.
...
- Проверки и оставшиеся ограничения: 51 адресный тест, ESLint, Prettier и
  сборка API. Повторение не гарантирует успех при длительной недоступности
  DeepSeek; ручное продолжение остаётся доступно и использует контрольные точки.
- Коммит реализации: нет. Выкладка: не было.
- Восстановление: прежний API-образ; миграций и backfill нет.
