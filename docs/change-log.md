...
- Проверки и оставшиеся ограничения: 9 адресных тестов, ESLint и сборка API
  проходят. Изолированный контейнер на хостинге без входа собрал 11 текстов
  из 297 отзывов контрольной карточки; повторный запуск получил только обзор,
  так как Google скрыл вкладку. Опубликованный интерфейс ещё не проверен.
- Коммит реализации: не создан.
- Выкладка: не было.
- Восстановление: возврат прежнего API/web-образа, откат БД не нужен.
...
- Проверки и оставшиеся ограничения: адресный ESLint прошёл.
