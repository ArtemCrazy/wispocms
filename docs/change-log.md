...
- Проверки: 11 адресных web-тестов, TypeScript, адресный ESLint и `diff --check`
  прошли. В production Browser напрямую подтверждён опубликованный SVG HTML
  (`viewBox 0 0 32 32`, четыре контура). Полная повторная проверка внутри списка
...
- Финальный коммит реализации и выкладка:
  `5315bf3765f3803c8a8ae658f83bb937ae894279`; промежуточный серый вариант
  `6931268e6095dd2514e2aa25a62d1ae2730e999f` заменён. Web-контейнер обновлён
...
- Восстановление: исходники до финальной выкладки сохранены в
  `/opt/wispo-cms-releases/wispo-cms-20260921-pre-html-icon`, прежний web-образ —
  `wispo-cms-web:pre-html-icon`; изменение БД для отката не требуется.
