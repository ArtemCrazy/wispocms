...
- Проверки и оставшиеся ограничения: 51 адресный тест, ESLint, Prettier и
  ...
  старого запуска; отдельная локальная PostgreSQL-база не настроена. Повторение
  не гарантирует успех при длительной недоступности DeepSeek; ручное
  продолжение остаётся доступно и использует контрольные точки.
