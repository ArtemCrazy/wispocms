...
- Статус: **В работе**. Владелец: задача Артёма / Codex; ветка `feature/content-center-preparation-mvp`.
- План: отдельная настройка Groq Whisper (не DeepSeek и не YouTube Data API), зашифрованное хранение ключа, проверка соединения, очередь по видео YouTube, повторяемые задания и сохранение текста расшифровки в снимке источника.
- БД — схема: будет добавлена отдельная миграция для настройки провайдера и очереди заданий; существующие ключи, материалы и версии не переписываются.
- Выкладка: пока не выполнялась. Перед выкладкой обязательны миграция, API/web проверки и проверка очереди на тестовом видео.
