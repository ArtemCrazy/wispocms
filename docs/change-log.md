...
- Проверки и оставшиеся ограничения: 4 профильных теста формы сайта, ESLint
  изменённого TSX-файла и `diff --check` прошли. Авторизованный Browser-сценарий
  недоступен из-за истёкшей production-сессии; визуальное изменение ограничено
  одним CSS-классом первого поля.
- Коммит реализации: ожидается.
- Выкладка: не выполнялась.
- Восстановление: возврат CSS-класса восстановит прежний увеличенный отступ.
...
- Приложение на общем сервере: `6cbb64646fbbcb017dc8edb2ddee1440ef17bbab`.
