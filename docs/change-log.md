...
- Что изменяется: только отображение сохранённого YouTube-канала в плашке —
...
- Проверки: 11 адресных тестов отображения прошли, web TypeScript и адресный ESLint прошли;
  визуальный smoke требуется после выкладки.
- Коммит реализации: пока нет. Выкладка: пока не выполнялась.
