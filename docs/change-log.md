...
- Статус: **Готово, не выложено (коррекция иконки)**. Владелец: задача Артёма / Codex,
...
- Проверки: 11 адресных web-тестов, TypeScript, адресный ESLint и `diff --check`
  прошли. В production Browser у сохранённого сайта подтверждён первоначальный `/globe.svg`;
  цветной вариант ожидает повторной проверки после корректирующей выкладки,
