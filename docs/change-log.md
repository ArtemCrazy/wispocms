...
- Статус: **Интеграционный кандидат; выпуск заблокирован** до согласования доступа контент-центра и единой публикации. Владелец: задача Артёма / Codex; ветка `integration/roman-content-center-20260923`.
...
- Merge-коммит: `88b6e6f`; опубликован [draft PR #2](https://github.com/ArtemCrazy/wispocms/pull/2). Документационное уточнение проверки: Browser подтвердил только frontend/экран входа, без связанного локального API; изолированный API проверен отдельно. Тестовые копии удалены, main и рабочая БД не изменены.
