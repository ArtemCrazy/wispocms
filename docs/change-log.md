...
- Статус: **Готово к тестовой выкладке**.
...
- Что изменено и зачем: перенос действующего Wispo CMS с общего preview-сервера
  на отдельный VDS заказчика после проверки резервных копий и целевого окружения.
- Файлы / модули: инфраструктура развёртывания; конкретные изменения будут
  зафиксированы по завершении.
- БД — схема: изменений кода схемы пока нет; при переносе сверить миграции.
- БД — данные и формат: планируется перенос действующей БД без замены другим
  дампом; исходную БД до подтверждённого переключения не изменять.
- Совместимость / пересечения: старый сервер общий с другими проектами;
  затрагивать только ресурсы Wispo CMS.
- Проверки и оставшиеся ограничения: новый VDS `129.101.122.78` настроен;
  Docker-копия с БД и медиа запущена на отдельном HTTPS-адресе. Архивы
  сверены по SHA-256; 3 контейнера здоровы; браузерный вход администратора,
  материалы и история версий проверены; из одного DOCX успешно создана новая
  тестовая версия. Локальный архив на новом VDS восстановлен в отдельную БД.
  Проверка Groq по IPv6 подтвердила ключ и модель, обновление YouTube собрало
  46 материалов; полный цикл аудио ещё не проверен. Нужны домен заказчика,
  независимое хранилище бэкапов и
  финальный свежий перенос. SSH root переведён на вход только по ключу.
- Коммит реализации: нет.
- Выкладка: тестовая копия на `https://wispo-cms.129.101.122.78.nip.io/`;
  рабочий `cms.kpbox.ru` не переключался.
- Восстановление: старое окружение сохранить до проверки нового и отката.
