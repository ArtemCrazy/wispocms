...
- Проверки и оставшиеся ограничения: `diff --check` прошёл; production-сборка
  web проверит CSS Modules перед переключением контейнера.
- Коммит реализации: ожидается.
- Выкладка: не выполнялась.
- Восстановление: удаление правил scrollbar вернёт системное оформление браузера.
...
- Приложение на общем сервере: `f976a5780f2d57739bf20ae7cba481448ea982f8`.
