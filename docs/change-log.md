...
## Проверенная точка отсчёта — 20.09.2026

- Наша ветка: `feature/content-center-preparation-mvp`.
- Приложение на общем сервере: `9f169b776aa02b2594ff43f54dc428bf8f00d3a7`.
  Сверено чтением `/opt/wispo-cms/.deployed-commit` 20.09.2026.
...
### 2026-09-21 · Универсальная иконка сайта в списке источников

- Статус: **Выложено**. Владелец: задача Артёма / Codex, ветка `feature/content-center-preparation-mvp`.
- Что изменено: у источников категории «Сайт» появилась нейтральная иконка
  глобуса в той же позиции и размере, что брендовые значки социальных сетей.
...
- Проверки: 11 адресных web-тестов, TypeScript, адресный ESLint и `diff --check`
  прошли. В production Browser у сохранённого сайта подтверждены `/globe.svg`,
  прежний текст URL и прежнее доступное название кнопки.
- Коммит реализации и выкладка: `6931268e6095dd2514e2aa25a62d1ae2730e999f`,
  web-контейнер обновлён 21.09.2026; API и БД не перезапускались. Оба домена и
  штатный монитор прошли; 497 серверных файлов совпали с коммитом.
- Восстановление: исходники до выкладки сохранены в
  `/opt/wispo-cms-releases/wispo-cms-20260921-pre-site-icon`, прежний web-образ —
  `wispo-cms-web:pre-site-icon`; изменение БД для отката не требуется.
