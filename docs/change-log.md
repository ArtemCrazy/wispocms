...
- Статус: **Готово, не выложено**. Владелец: задача Артёма / Codex, ветка `feature/content-center-preparation-mvp`.
...
- БД — схема: новая миграция `1791091200000-InstagramYoutubeSources` в `data-source.ts`;
  ...
  Миграция применена на изолированной тестовой PostgreSQL, на общем сервере пока нет.
...
- Совместимость: сервер всё ещё `9f169b7`, 35 применённых миграций; конфигурация nginx
  совпала с канонической. После fetch в `origin/main` нет новой миграции/пересечения
...
- Выкладка: пока не выполнялась; перед изменением общей БД сверить сервер/миграции,
  проверить новую миграцию на отдельной БД и сохранить свежую резервную копию.
