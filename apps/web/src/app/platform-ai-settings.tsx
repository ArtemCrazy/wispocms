...
export function PlatformAiSettings({
  ...
  async function act(action: "save" | "check" | "remove") {
    ...
    if (
      action === "remove" &&
      !window.confirm(
        "Удалить сохранённый ключ DeepSeek? Новые AI-запуски станут недоступны. Уже созданные результаты сохранятся.",
      )
    )
      return;
    setBusy(action);
    ...
    try {
      ...
      setMessage(
        action === "save"
          ? "Ключ и модель сохранены. Теперь проверьте подключение."
          : action === "remove"
            ? "Ключ удалён. Существующие результаты сохранены."
            : "Ключ принят, модель доступна. Материалы не отправлялись. Можно перейти к тестовому запуску.",
      );
    } catch (reason) {
      setError(
        reason instanceof Error
  ...
  return (
    <section className={styles.root} aria-label="Настройки платформы">
      <header>
        <h1>Настройки платформы</h1>
        <p>
          Подключения и общая библиотека промптов для всех рабочих пространств.
      ...
      {!settings ? (
        <p>
          {error
            ? "Обновите страницу, чтобы повторить загрузку."
            : "Загружаем настройки…"}
        </p>
      ) : (
        <article className={styles.card}>
          <div className={styles.heading}>
            <div>
              <h2>DeepSeek</h2>
              <p>Подготовка информации и создание контента</p>
            </div>
          ...
          {!settings.storageReady && (
            <p className={styles.error} role="alert">
              Серверное хранилище ключей не настроено. Сохранение и запуск AI
              недоступны.
            </p>
          )}
          <form onSubmit={save}>
            <fieldset disabled={Boolean(busy) || !settings.storageReady}>
              <label htmlFor="deepseek-api-key">
                {settings.hasKey ? "Заменить API-ключ" : "API-ключ DeepSeek"}
              </label>
              <input
                id="deepseek-api-key"
                type="password"
                ...
                placeholder={
                  settings.hasKey
                    ? "Ключ сохранён. Для замены вставьте новый"
              ...
              />
              <p id="deepseek-key-help">
                Ключ хранится зашифрованным на сервере. Сохранённое значение не
                отображается и не возвращается в браузер. Пустое поле при смене
                модели сохраняет прежний ключ.
              </p>
              ...
              <div className={styles.actions}>
                <button
                  ...
                  disabled={!aiDirty || (!settings.hasKey && !apiKey.trim())}
                  type="submit"
                >
                ...
                <button
                  type="button"
                  disabled={!settings.configured || aiDirty}
          ...
          <div className={styles.details}>
            <p>
              Проверка подтверждает доступ к API и выбранной модели, но не
              запускает генерацию и не отправляет данные проектов.
            </p>
            <p>
              При запуске обработки текстовые материалы, задача и нужный
              контекст выбранного workspace передаются DeepSeek. Генерация
              оплачивается с вашего аккаунта DeepSeek. Сохранение ключа ничего
              не запускает автоматически.
            </p>
            <p>
              Сейчас поддерживаются текстовые материалы. PDF, Office,
              изображения и вложения в генерацию пока не поддерживаются. Ссылки
              сами по себе не загружаются; автоматический поиск источников не
              подключён.
            </p>
            {settings.verifiedAt && (
              <p>
                Последняя успешная проверка:{" "}
                {new Date(settings.verifiedAt).toLocaleString("ru-RU")}
              </p>
            )}
          </div>
          {settings.hasKey && (
            <button
              ...
              disabled={Boolean(busy) || aiDirty}
              onClick={() => void act("remove")}
            >
              {busy === "remove" ? "Удаляем…" : "Удалить ключ"}
            </button>
          )}
        </article>
      )}
