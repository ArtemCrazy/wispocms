...
describe('category revision routes', () => {
  ...
  const content = {
    assertVersionedCategory: jest.fn().mockResolvedValue(undefined),
    getCategoryRevisionPreview: jest
      .fn()
      .mockResolvedValue({ category: { id: categoryId } }),
    publishCategoryRevision: jest.fn().mockResolvedValue({ id: categoryId }),
  };
  ...
  it('exposes exact current, history, preview and publish', async () => {
    const base = `/api/sites/${siteId}/content/categories/${categoryId}/revisions`;
  ...
  it('exposes exact submit, approve, changes and restore with validated body', async () => {
    const base = `/api/sites/${siteId}/content/categories/${categoryId}/revisions/${revisionId}`;
    await request(app.getHttpServer() as App)
      .post(`${base}/submit`)
      .expect(201);
    await request(app.getHttpServer() as App)
