...
describe('author revision routes', () => {
  ...
  const content = {
    assertVersionedAuthor: jest.fn().mockResolvedValue(undefined),
    getAuthorRevisionPreview: jest.fn().mockResolvedValue({ id: authorId }),
    publishAuthorRevision: jest.fn().mockResolvedValue({ id: authorId }),
  };
  ...
  it('exposes exact current, history, preview and publish', async () => {
    const base = `/api/sites/${siteId}/content/authors/${authorId}/revisions`;
  ...
  it('exposes exact submit, approve, changes and restore with validated body', async () => {
    const base = `/api/sites/${siteId}/content/authors/${authorId}/revisions/${revisionId}`;
    await request(app.getHttpServer() as App)
      .post(`${base}/submit`)
      .expect(201);
    await request(app.getHttpServer() as App)
