  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `INSERT INTO "sites" ("id", "workspace_id", "created_by_user_id", "name", "slug", "domain", "site_type", "seo_title", "seo_description", "no_index", "global_data", "layout_settings", "is_active")
       VALUES ($1, $2, (SELECT "id" FROM "users" WHERE "platform_role" = 'wispo_admin' ORDER BY "created_at" LIMIT 1), $3, $4, NULL, 'media', $5, $6, false, $7::jsonb, $8::jsonb, true)`,
      [
        SITE.id,
        WORKSPACE.id,
        SITE.name,
        ...
        [
          {
            ...
            text: '# Политика конфиденциальности\n## Общие положения\nSkinova обрабатывает персональные данные только для ответа на обращения пользователей и работы консультационной формы.\n## Какие данные мы получаем\nИмя, номер телефона и содержание обращения, которые пользователь предоставляет добровольно.\n## Цели и сроки обработки\nДанные используются для связи по запросу пользователя и хранятся не дольше, чем требуется для этой цели.\n## Права пользователя\nПользователь может запросить уточнение или удаление своих данных у владельца сайта.',
          },
        ],
      ],
      [
    ...
    await queryRunner.query(
      `INSERT INTO "page_banner_assignments" ("site_id", "page_id", "banner_id", "zone") VALUES ($1, $2, $3, 'homepage_top'), ($1, $2, $4, 'homepage_middle')`,
      [SITE.id, pages[0][0], banners[0][0], banners[1][0]],
    );
    await queryRunner.query(
